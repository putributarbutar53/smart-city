...
<script>
    function dataindex() {
        $('#table_index').DataTable({
            ...
            'ajax': {
                'url': '<?= site_url('admin2053/category/loaddata') ?>',
                'headers': {
    ...
    function deletedata(iddata) {
        ...
        $("#click_yes").off("click").on("click", function() {
            $.ajax({
                type: 'DELETE',
                url: "<?= site_url('admin2053/category/delete') ?>/" + iddata,
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
    ...
    function editdata(iddata) {
        $.get("<?= site_url('admin2053/category/edit') ?>/" + iddata, function(data, status) {
    ...
    function adddata() {
        $('#editor_add').load('<?= site_url('admin2053/category/add') ?>', function() {
            $('#add').modal({
                show: true
            });
        });
    ...
    function detaildata(iddata) {
        $.get("<?= site_url('admin2053/category/detail') ?>/" + iddata, function(data, status) {
